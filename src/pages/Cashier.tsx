...
import { db, type Product, type Category, type Transaction, type TransactionItemRecord, type ProductVariant } from '@/lib/db';
...
const [receiptOpen, setReceiptOpen] = useState(false);
  ...
  const addToCart = async (product: Product) => {
    if (product.hasVariants === 1) {
      const vars = await db.productVariants
        .where('productId').equals(product.id!)
        .filter(v => v.isDeleted === 0 && v.stock > 0)
        .toArray();
      
      if (vars.length === 0) {
        toast.error('Semua varian stoknya habis');
        return;
      }
    ...
    setCart(prev => {
      const existing = prev.find(c => c.product.id === product.id && !c.variant);
      if (existing) {
        if (existing.qty >= product.stock) {
          toast.error('Stok tidak cukup');
          return prev;
        }
        return prev.map(c => (c.product.id === product.id && !c.variant) ? { ...c, qty: c.qty + 1 } : c);
      }
  ...
  const addVariantToCart = (product: Product, variant: ProductVariant) => {
    setCart(prev => {
      ...
      if (existing) {
        ...
        return prev.map(c => c.variant?.id === variant.id ? { ...c, qty: c.qty + 1 } : c);
      }
  ...
  const updateQty = (cartIndex: number, delta: number) => {
    setCart(prev => prev.map((c, i) => {
      ...
      if (newQty > maxStock) { toast.error('Stok tidak cukup'); return c; }
      return { ...c, qty: newQty };
    }));
  };
  ...
  const handleCheckout = async () => {
    ...
    const itemRecords: TransactionItemRecord[] = cart.map(c => ({
      ...
      productName: c.variant ? `${c.product.name} (${c.variant.name})` : c.product.name,
      quantity: c.qty,
      price: c.variant ? c.variant.price : c.product.price,
      hpp: c.variant ? c.variant.hpp : c.product.hpp,
      discountType: c.discountType,
      discountValue: c.discountValue,
      discountAmount: c.discountType === 'percentage' ? (c.variant ? c.variant.price : c.product.price) * c.qty * c.discountValue / 100 : c.discountType === 'nominal' ? c.discountValue : 0,
    ...
    for (const item of cart) {
      if (item.variant?.id) {
        await db.productVariants.update(item.variant.id, { stock: item.variant.stock - item.qty });
      } else {
                ...
                <CardContent className="p-0">
                  ...
                  <div className="p-2.5">
                    <div className="flex items-center justify-between gap-1">
                      <h3 className="text-xs font-semibold truncate flex-1">{p.name}</h3>
                      {p.hasVariants === 1 && (
                        <Badge variant="secondary" className="text-[9px] px-1 h-3.5 bg-primary/10 text-primary border-none">
                          Varian
                        </Badge>
                      )}
                    </div>
                ...
                <div key={item.product.id} className="flex items-center gap-3 bg-muted/50 p-3 rounded-xl">
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-muted-foreground">
                      Rp {(item.variant ? item.variant.price : item.product.price).toLocaleString('id-ID')} × {item.qty}
                    </p>
                    <p className="text-sm font-bold text-primary">Rp {getItemSubtotal(item).toLocaleString('id-ID')}</p>
                  </div>
                  <div className="flex items-center gap-1">
                    <Button variant="outline" size="icon" className="h-8 w-8 rounded-full" onClick={() => item.qty === 1 ? removeFromCart(cart.indexOf(item)) : updateQty(cart.indexOf(item), -1)}>
                      {item.qty === 1 ? <X className="w-3 h-3" /> : <Minus className="w-3 h-3" />}
                    </Button>
                    <span className="w-8 text-center text-sm font-bold">{item.qty}</span>
                    <Button variant="outline" size="icon" className="h-8 w-8 rounded-full" onClick={() => updateQty(cart.indexOf(item), 1)}>
...
{/* Receipt Dialog */}
      ...
      <Dialog open={variantDialogOpen} onOpenChange={setVariantDialogOpen}>
        <DialogContent className="max-w-[95vw] rounded-xl">
          <DialogHeader>
            <DialogTitle className="text-base">Pilih Varian: {activeProduct?.name}</DialogTitle>
          </DialogHeader>
          <div className="grid grid-cols-2 gap-3 mt-2">
            {productVariants.map(v => (
              <button
                key={v.id}
                onClick={() => activeProduct && addVariantToCart(activeProduct, v)}
                className="flex flex-col items-start p-3 rounded-xl border-2 border-muted hover:border-primary/50 hover:bg-primary/5 transition-all text-left group"
              >
                <span className="text-sm font-bold group-hover:text-primary transition-colors">{v.name}</span>
                <span className="text-xs font-semibold text-primary mt-1">Rp {v.price.toLocaleString('id-ID')}</span>
                <span className="text-[10px] text-muted-foreground mt-0.5">Stok: {v.stock}</span>
              </button>
            ))}
          </div>
        </DialogContent>
