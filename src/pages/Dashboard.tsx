...
export default function Dashboard() {
  ...
  const todayTransactions = useLiveQuery(async () => {
    return db.transactions.where('date').aboveOrEqual(today).filter(t => t.isCanceled !== 1).toArray();
  }, []);

  const lowStockProducts = useLiveQuery(() => db.products.filter(p => p.isDeleted === 0 && p.stock <= 5).toArray());

  const recentTransactions = useLiveQuery(() =>
    db.transactions.where('isCanceled').equals(0).reverse().limit(5).toArray()
  );
  ...
  return (
    <div className="px-4 pt-6 space-y-5">
      ...
      <div>
        ...
        <h1 className="text-2xl font-bold tracking-tight">{storeSettings?.storeName || 'FreePOS'}</h1>
      </div>
