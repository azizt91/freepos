...
export interface Product {
  ...
  deletedAt: Date | null;
  hasVariants: number; // 0 = false, 1 = true
}

export interface ProductVariant {
  id?: number;
  productId: number;
  name: string; // Contoh: "Merah, S"
  sku: string;
  barcode?: string;
  price: number;
  hpp: number;
...
export interface Transaction {
  ...
  remarks?: string;
  isCanceled?: number; // 0 = active, 1 = canceled
...
class PosDatabase extends Dexie {
  ...
  constructor() {
    ...
    // Version 3 — Product Variants
    this.version(3).stores({
      products: '++id, name, sku, categoryId, barcode, isDeleted, hasVariants',
      productVariants: '++id, productId, sku, barcode, isDeleted',
      transactionItems: '++id, transactionId, productId, variantId',
      stockIns: '++id, productId, supplierId, variantId, date',
      stockOuts: '++id, productId, variantId, date',
    }).upgrade(async (tx) => {
      // Set hasVariants default on existing products
      await tx.table('products').toCollection().modify(p => {
        p.hasVariants = 0;
      });
    });

    // Version 4 — Cancel Transaction
    this.version(4).stores({
      transactions: '++id, date, &receiptNumber, paymentMethodId, isCanceled',
    }).upgrade(async (tx) => {
